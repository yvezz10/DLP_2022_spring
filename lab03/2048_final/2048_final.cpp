...
public:
	...
	virtual float update(const board& b, float u) {
		//TODO
		float value = 0;
		float u_update = u/iso_last;
		for (int i = 0; i< iso_last; i++){
			size_t index = indexof(isomorphic[i], b);
			operator[](index) = operator[](index) + u_update;
			value = value + operator[](index);
		}
		return value;
	}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				//float value = move->reward() + estimate(move->after_state());
				...
				for (int i =0; i<16;i++){
					if (s_next.at(i) == 0){
						empty_size++;
						s_next.set(i,1);
						value += estimate(s_next)*0.9;
						s_next.set(i,2);
						value += estimate(s_next)*0.1;
						s_next.set(i,0);
					}
				}
				if (empty_size != 0){
					value = value/empty_size;
				}
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		float episodeValue = 0 - estimate(move.before_state());
		while((path.size()-1)!=0){
			path.pop_back();
			state& move = path.back();
			float error = alpha*(episodeValue - estimate(move.before_state()) + move.reward());
			episodeValue =  update(move.before_state(), error);
		}
	}
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			myFile.open("score_log_final.txt", std::fstream::app);
...
int main(int argc, const char* argv[]) {
	...
	std::fstream myFile;
	myFile.open("score_log_final.txt", std::fstream::out|std::fstream::trunc);
	myFile<<"episode"<<"\t"<<"win_rate"<<"\t"<<"score"<<std::endl;
	myFile.close();
	...
	//new add 
	tdl.add_feature(new pattern({ 0, 4, 8, 9, 10, 13}));
	tdl.add_feature(new pattern({ 0, 4, 5, 6, 2, 3}));
	tdl.add_feature(new pattern({ 0, 4, 5, 9, 10, 11}));	
	tdl.add_feature(new pattern({ 0, 1, 2, 6, 9, 10}));
	tdl.add_feature(new pattern({ 1, 4, 5, 6, 9, 13}));

	tdl.add_feature(new pattern({ 6, 8, 9, 10, 13 }));
	tdl.add_feature(new pattern({ 1, 2, 5, 6, 7 }));
	tdl.add_feature(new pattern({ 2, 4, 5, 6, 9 }));
	tdl.add_feature(new pattern({ 0, 1, 2, 3, 4 }));
	tdl.add_feature(new pattern({ 0, 1, 2, 6, 7 }));
	tdl.add_feature(new pattern({ 4, 5, 6, 10, 11 }));
	tdl.add_feature(new pattern({ 0, 1, 2, 4, 6 }));
	tdl.add_feature(new pattern({ 0, 1, 5, 9, 10 }));

	tdl.add_feature(new pattern({ 0, 1, 2, 6 }));
	tdl.add_feature(new pattern({ 1, 2, 3, 7 }));
	tdl.add_feature(new pattern({ 1, 2, 6, 10}));
	tdl.add_feature(new pattern({ 4, 5, 6, 9 }));
    tdl.add_feature(new pattern({ 0, 1, 5, 6 }));
	tdl.add_feature(new pattern({ 1, 4, 5, 6 }));

	// restore the model from file
	tdl.load("weight_final");
...
//86/93/98
// start running at 2022/04/01 13:55